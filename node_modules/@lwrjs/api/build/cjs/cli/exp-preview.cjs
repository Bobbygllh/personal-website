...
async function expPreview(options) {
  ...
  try {
    ...
    if (process.env.PROXY_FOR_SSR === void 0)
    ...
    if (proxyConfig) {
      ...
      await (0, import_utils.setupProxy)(runtimePort, proxyPort, options.basePath, {...proxyConfig, remoteOrigin});
    }
    if (target === "mrt") {
      const ssrJs = import_path.default.join(dir, "ssr.js");
      if ((0, import_shared_utils.isLocalAuthEnabled)()) {
        (0, import_setup_dev.injectLocalDevHacks)(import_path.default.join(dir, "experience", "index_csr.html"));
        (0, import_setup_dev.injectLocalDevHacks)(import_path.default.join(dir, "experience", "index.html"));
      }
      if (import_fs_extra.default.existsSync(ssrJs)) {
        (0, import_child_process.spawn)("node", [`${ssrJs}`], {
          ...
          env: {
            ...
            NODE_ENV: "test",
            NODE_OPTIONS: [process.env.NODE_OPTIONS, options?.debug ? "--inspect-brk" : ""].filter(Boolean).join(" ").trim()
          },
          stdio: "inherit"
        });
      } else {
    ...
    console.log(import_chalk.default.green(`Application is available at: ${serverUrl}`));
    if (open && target !== "mrt") {
      await (0, import_shared_utils.launch)(runtimePort, void 0, options.basePath);
    }
  } catch (error) {
