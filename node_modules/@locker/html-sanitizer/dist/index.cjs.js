...
const queue$LWS = shared$LWS.toSafeSet(new shared$LWS.SetCtor());
...
function createSantizerHooksRegistry$LWS(sandboxKey$LWS) {
  return {
    ...
    uponSanitizeElement(node$LWS, data$LWS, config$LWS) {
      ...
      if (tagNameCheck$LWS && shared$LWS.ReflectApply(shared$LWS.RegExpProtoTest, tagNameCheck$LWS, [tagName$LWS]) &&
...
function sanitizeSvgHref$LWS(url$LWS) {
  const urlAsString$LWS = shared$LWS.toSafeStringValue(url$LWS);
  ...
  if (sharedUrl$LWS.URL_SCHEMES_LIST.includes(normalizedHref$LWS.protocol)) {
    const container$LWS = shared$LWS.ReflectApply(sharedDom$LWS.DocumentProtoGetElementById, document, [normalizedHref$LWS.normalizedURL]);
...
function updater$LWS(container$LWS, normalizedHref$LWS) {
  ...
  let el$LWS = shared$LWS.ReflectApply(sharedDom$LWS.ElementProtoQuerySelector, container$LWS, [`#${normalizedFragment$LWS}`]);
