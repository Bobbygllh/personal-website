...
const bImportDeclaration = (imports, source = '@lwc/ssr-runtime') => {
    ...
    const specifiers = parsed.map(([imported, local]) => {
        if (imported === 'default') {
            return builders.importDefaultSpecifier(builders.identifier(local));
        }
        else if (imported === '*') {
            return builders.importNamespaceSpecifier(builders.identifier(local));
        }
        else if (local) {
            return builders.importSpecifier(builders.identifier(imported), builders.identifier(local));
        }
        else {
            return builders.importSpecifier(builders.identifier(imported));
        }
    });
    return builders.importDeclaration(specifiers, builders.literal(source));
};
...
function replaceLwcImport(path, state) {
    ...
    path.replaceWith(builders.importDeclaration(structuredClone(path.node.specifiers), builders.literal('@lwc/ssr-runtime')));
}
/**
...
function replaceNamedLwcExport(path) {
    ...
    path.replaceWith(builders.exportNamedDeclaration(structuredClone(path.node.declaration), structuredClone(path.node.specifiers), builders.literal('@lwc/ssr-runtime')));
}
/**
...
function replaceAllLwcExport(path) {
    ...
    path.replaceWith(builders.exportAllDeclaration(builders.literal('@lwc/ssr-runtime'), structuredClone(path.node.exported)));
}
/**
...
function catalogAndReplaceStyleImports(path, state) {
    ...
    if (path.node?.source.value.endsWith('.scoped.css')) {
        path.replaceWith(builders.importDeclaration(path.node.specifiers, builders.literal(path.node.source.value + '?scoped=true')));
    }
...
function bMemberExpressionChain(props) {
    ...
    let expr = builders.identifier('instance');
    for (const prop of props) {
        expr = builders.memberExpression(expr, builders.literal(prop), true);
    }
    return expr;
}
...
function validateWireId(id, path) {
    ...
    if (path.scope?.getBinding(wireAdapterVar)?.kind !== 'module') {
        throw generateError(path.node, DecoratorErrors.COMPUTED_PROPERTY_MUST_BE_CONSTANT_OR_LITERAL);
    }
}
...
function catalogWireAdapters(path, state) {
    ...
    if (config) {
        ...
        reactiveConfig = builders.objectExpression([]); // empty object
    }
    state.wireAdapters = [
        ...state.wireAdapters,
...
function removeDecoratorImport(path) {
    ...
    if (filteredSpecifiers.length !== path.node.specifiers.length) {
        path.replaceWith(builders.importDeclaration(filteredSpecifiers, builders.literal('@lwc/ssr-runtime')));
    }
}
...
const visitors = {
    ...
    ImportExpression(path, state) {
        ...
        const load = builders.identifier('__lwcLoad');
        state.trustedLwcIdentifiers.add(load);
        path.replaceWith(builders.callExpression(load, [structuredClone(source)]));
    },
    ClassDeclaration: {
        enter(path, state) {
            const { node } = path;
        ...
        switch (node.key.name) {
            case 'constructor':
                ...
                node.value.params = [
                    ...
                    builders.identifier('propsAvailableAtConstruction'),
                ];
                break;
...
function getChildAttrsOrProps(attrs, cxt) {
    const objectAttrsOrProps = attrs
        ...
        if (value.type === 'Literal' && typeof value.value === 'string') {
            ...
            return builders.property('init', key, builders.literal(type === 'Attribute' ? '' : value.value));
        }
        ...
        .filter(Boolean);
    return builders.objectExpression(objectAttrsOrProps);
}
/**
...
function generateExpressionFromTextNode(node, cxt) {
    return isLiteral(node.value) ? builders$1.literal(node.value.value) : expressionIrToEs(node.value, cxt);
}
function generateConcatenatedTextNodesExpressions(cxt) {
    ...
    const concatenatedExpression = textNodes
        ...
        .reduce((accumulator, expression) => builders$1.binaryExpression('+', accumulator, expression));
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            switch (node.type) {
                ...
                case 'ExternalComponent': {
                    ...
                    const slotName = node.type === 'Text' ? builders.literal('') : bAttributeValue(node, 'slot');
...
function getSlottedContent(node, cxt) {
    ...
    const scopedSlotContent = scopedSlottableChildren.map((child) => {
        ...
        const boundVariable = builders.identifier(boundVariableName);
...
const Component = function Component(node, cxt) {
    ...
    return [
        bYieldFromChildGenerator(getChildAttrsOrProps(node.properties, cxt), getChildAttrsOrProps(node.attributes, cxt), getSlottedContent(node, cxt), builders.identifier(childComponentLocalName), builders.literal(childTagName)),
    ];
};
...
function yieldAttrOrPropLiteralValue(name, valueNode) {
    ...
    if (typeof value === 'string') {
        ...
        return [bStringLiteralYield(builders.literal(name), builders.literal(yieldedValue))];
    }
    else if (typeof value === 'boolean') {
        if (name === 'class') {
            return [];
        }
        return [bYield(builders.literal(` ${name}`))];
    }
...
function yieldAttrOrPropDynamicValue(elementName, name, value, cxt) {
    ...
    switch (name) {
        ...
        default:
            return [
                bYieldDynamicValue(builders.literal(name), scopedExpression, builders.literal(isBooleanAttribute(name, elementName))),
            ];
    }
}
...
const Element = function Element(node, cxt) {
    ...
    return [
        ...
        ...yieldAttrsAndProps,
        bYield(builders.literal(isForeignSelfClosingElement ? `/>` : `>`)),
        ...(isSelfClosingElement ? [] : [...childContent, bYield(builders.literal(`</${node.name}>`))]),
    ].filter(Boolean);
};
...
function bYieldBookendComment() {
    return builders.expressionStatement(builders.yieldExpression(builders.literal(`<!---->`)));
}
...
function irToEs(node, cxt) {
    if ('directives' in node && node.directives.some((d) => d.name === 'Dynamic')) {
        return [
            bThrowError(builders.literal('The lwc:dynamic directive is not supported for SSR. Use <lwc:component> instead.')),
        ];
    }
...
function compileTemplate(src, filename, options, compilationMode) {
    const { root, warnings } = parse$2(src, {
 ...
 * For full license text, see the LICENSE file in the repo root or https://opensource.org/licenses/MIT
 */
function compileComponentForSSR(src, filename, options, mode = DEFAULT_SSR_MODE) {
...
function compileTemplateForSSR(src, filename, options, mode = DEFAULT_SSR_MODE) {
