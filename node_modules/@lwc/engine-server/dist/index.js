...
function setProperty(node, propName, value) {
    ...
    if (process.env.NODE_ENV !== 'production') {
        // eslint-disable-next-line no-console
        console.error(`Unexpected attempt to set "${propName}=${value}" property from the renderer`);
    }
}
...
function getAttribute(element, name, namespace = null) {
    ...
    const attribute = element[HostAttributesKey].find((attr) => attr.name === normalizedName && attr[HostNamespaceKey] === namespace);
...
function removeAttribute(element, name, namespace) {
    const normalizedName = StringToLowerCase.call(String(name));
    reportMutation(element, normalizedName);
    element[HostAttributesKey] = element[HostAttributesKey].filter((attr) => attr.name !== normalizedName && attr[HostNamespaceKey] !== namespace);
}
...
export { LightningElement, SignalBaseClass, addTrustedContext as __dangerous_do_not_use_addTrustedContext, api$1 as api, createContextProvider, createElement, freezeTemplate, getComponentDef, hot, isComponentConstructor, isTrustedSignal, parseFragment, parseFragment as parseSVGFragment, readonly, registerComponent, registerDecorators, registerTemplate, renderComponent, renderer, sanitizeAttribute, setContextKeys, setFeatureFlag, setFeatureFlagForTest, setHooks, setTrustedContextSet, swapComponent, swapStyle, swapTemplate, track, unwrap, wire };
/** version: 8.22.4 */
