...
var moduleResolver = require('@lwc/module-resolver');
...
function isImplicitHTMLImport(importee, importer, importerExt) {
    return (SCRIPT_FILE_EXTENSIONS.includes(importerExt) &&
...
function transformWarningToRollupLog(warning, src, id) {
...
function lwc(pluginOptions = {}) {
    ...
    const { targetSSR, ssrMode, stylesheetConfig, sourcemap = false, preserveHtmlComments, experimentalDynamicComponent, experimentalDynamicDirective, enableDynamicComponents, enableLwcOn, enableLightningWebSecurityTransforms, 
    ...
    return {
        ...
        resolveId(importee, importer) {
            if (importer) {
                ...
                if (importee.startsWith('.') || importee.startsWith('/')) {
                    ...
                    if (isImplicitHTMLImport(importeeNormalizedFilename, importerFilename, importerExt) &&
                    ...
                    try {
                        const { entry, specifier, type } = moduleResolver.resolveModule(importee, importer, {
                            modules,
                            rootDir,
                        });
                        if (type === moduleResolver.RegistryType.alias) {
                            ...
                            const hasValidSpecifier = !!namespace?.length && !!name?.length && !rest?.length;
        ...
        transform(src, id) {
            ...
            if (!namespace || !name) {
                ...
                this.warn('The component namespace and name could not be determined from the specifier ' +
                    JSON.stringify(specifier) +
                    ' or filename ' +
                    JSON.stringify(filename));
            }
            const apiVersionToUse = shared.getAPIVersionFromNumber(apiVersion);
            const { code, map, warnings } = compiler.transformSync(src, filename, {
                name,
                namespace,
