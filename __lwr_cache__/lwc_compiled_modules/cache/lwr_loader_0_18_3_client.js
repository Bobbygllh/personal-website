...
(function (MODULE_WARNING) {
  const MODULE_REDEFINE = 'Module redefine attempted';
  MODULE_WARNING["MODULE_REDEFINE"] = MODULE_REDEFINE;
  const MODULE_ALREADY_LOADED = 'Marking module(s) as externally loaded, but they are already loaded';
  MODULE_WARNING["MODULE_ALREADY_LOADED"] = MODULE_ALREADY_LOADED;
  const ALIAS_UPDATE = 'Alias update attempt';
  MODULE_WARNING["ALIAS_UPDATE"] = ALIAS_UPDATE;
...
class ModuleRegistry {
  ...
  constructor(config) {
    ...
    ModuleRegistry.prototype.__init4.call(this);
